...
<div class="navbar">
   ...
   <form class="navbar-search">
        <input type="text" name="Search" method="POST" class="navbar-search-input" placeholder="Chercher un etudiant">
...
<div class="wrapper" style="margin-top:20px;">
                ...
                <div class="card-content">
                    <table>
                        ...
                        <tr>
                            ...
                            <th>telephone</th>
                            <th>Nationalité</th>
                            <th style="color:#FB9300;">Supprimer </th>
                            <th style="color:#FB9300;">Modifier </th>
                        
                        </tr>
                        </thead>

                        <?php   $con=new pdo("mysql:host=cefolie51.mysql.db; dbname=cefolie51","cefolie51","JgmgJJ3JYvYv");

                        require_once'phpqrcode/qrlib.php';
      require_once'phpqrcode/qrconfig.php';
      $get=$con->prepare("select * from client");
       $get->execute();
      foreach($get as $dat){
              $tel=$dat['Tel'];
              $id_c=$dat[0];
        ...
        echo'  <tr class="data">
               ...
               <td>
        <a style="color:#f00 ; text-align: center;margin-left:30px;" href="Pre-inscrits.php?del_user='.$dat['id'].'" title="Delete"><i class="fas fa-trash"></i></a>
              </td>
              <td>
    ...
    if(isset($_GET['del_user'])){
        $id_u=$_GET['del_user'];
        $del=$con->prepare("DELETE FROM client WHERE id='$id_u' ");
        if($del->execute()){
              echo"<script>alert('User Deleted ')</script>";
              echo"<script>window.open('Pre-inscrits.php','_self')</script>";
            }else
            {
...
function edit_data(id){
   ...
   var cin=$('#cin'+id).text();
    ...
    var daten=$('#daten'+id).text();
    var nationalite=$('#nationalite'+id).text();
    $.ajax({
        url:"edit.php",
        method:"POST",
        data:{id:id,cin:cin,nom:nom,prenom:prenom,email:email,daten:daten,nationalite:nationalite },
        dataType:"text",
    });
}
var c;
